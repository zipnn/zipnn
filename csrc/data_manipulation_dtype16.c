...
static void reorder_all_floats_dtype16(uint8_t *src, Py_ssize_t len) {
...
int split_bytearray_dtype16(uint8_t *src, Py_ssize_t len,
                            uint8_t **chunk_buffs,
...
// Helper function to reorder all floats in a bytearray
static void revert_all_floats_dtype16(uint8_t *src, Py_ssize_t len) {
  uint32_t *uint_array = (uint32_t *)src;
  Py_ssize_t num_floats = len / sizeof(uint32_t);
  for (Py_ssize_t i = 0; i < num_floats; i++) {
...
int combine_buffers_dtype16(const uint8_t *buf1, const uint8_t *buf2,
                            uint8_t *combinePtr, const Py_ssize_t *bufLens,
