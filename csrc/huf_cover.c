...
size_t hufCompressData(const uint8_t* data, size_t size, int maxCompressedSize, uint8_t* compressedData, size_t* compressedChunksSize, size_t chunkSize, float compThreshold, size_t checkThreshold) {
    ...
    size_t checkCompTh = size / checkThreshold; // check after checkTh% of the size, if not compressed sont comtinue
    uint8_t isCheck = 0;

    for (size_t offset = 0; offset < size; offset += chunkSize) {
	size_t curChunkSize = (size - offset > chunkSize) ? chunkSize : (size - offset);

	// Compress
        size_t compressedSize = HUF_compress(compressedData + totalCompressedSize, RemianCapacity, data + offset, curChunkSize);
...
size_t hufDecompressData(const uint8_t* compressedData, size_t* compressedChunksSize, size_t numChunks, size_t original_size, uint8_t* decompressedData, size_t chunkSize)
{
    ...
    for (size_t i = 0; i < numChunks; i++) {
        ...
        size_t expectedChunkSize = (remainingOriginalSize > chunkSize) ? chunkSize : remainingOriginalSize;
         
        // Decompress each chunk
        size_t decompressedSize = HUF_decompress(
            decompressedData + totalDecompressedSize,
            expectedChunkSize,
