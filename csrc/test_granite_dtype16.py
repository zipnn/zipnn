...
# byte_mode = 6 [ByteGrouping]
# is_review = 0 [no review]
# thread = 1 [one thread]
buf1, buf2 = split_dtype.split_dtype16(original_bytes, 1, 6, 0 ,1) # bit_mode = 1 [bit_ordering], 
...
print (f"% of comp2 {len(comp2)/len(original_bytes)*2}, {time.time() - start_time}")

if(len(comp2) > (len(original_bytes)/2)):
   len_comp2 = len(original_bytes)/2
else:
   len_comp2 = len(comp2)
print (f"% of comp_all {(len_comp1 + len_comp2)/len(original_bytes)}")

start_time = time.time()
revert = zstd.decompress(comp2)
print (f"% of revert comp2 {time.time() - start_time}")
zstd = ZipNN(bg_partitions = 1, zstd_threads=1)
...
# byte_mode = 6 [ByteGrouping]
# thread = 1 [one thread]
combined_bytes = split_dtype.combine_dtype16(buf1, buf2, 1, 6, 1)
